...
# python manage.py seed_day_of_week_defaults --property-code P6
# python manage.py seed_day_of_week_defaults --property-id 1000
# python manage.py seed_day_of_week_defaults --all


class Command(BaseCommand):
    help = 'Seed predefined day-of-week groups and mappings for one or more properties.'
    ...
    def handle(self, *args, **options):
        ...
        for property_obj in properties.order_by('id'):
            seed_default_day_of_week_for_property(property_obj, actor=None)
            self.stdout.write(
                self.style.SUCCESS(
                    f'Seeded day-of-week defaults for property {property_obj.code} - {property_obj.name}'
                )
            )