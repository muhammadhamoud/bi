...
from apps.settings.mappings.data_dontuse.origin_defaults import (
    seed_default_origins_for_property,
)

# python manage.py seed_origin_defaults --property-code P6
# python manage.py seed_origin_defaults --property-id 1000
# python manage.py seed_origin_defaults --all


class Command(BaseCommand):
    help = "Seed predefined origin groups, categories, and mappings for one or more properties."
    ...
    def handle(self, *args, **options):
        ...
        for property_obj in properties.order_by("id"):
            seed_default_origins_for_property(property_obj, actor=None)
            self.stdout.write(
                self.style.SUCCESS(
                    f"Seeded origin defaults for property {property_obj.code} - {property_obj.name}"
                )
            )