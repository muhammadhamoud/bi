...
from apps.settings.mappings.data_dontuse.competitor_defaults import (
    seed_default_competitors_for_property,
)

# python manage.py seed_competitor_defaults --property-code P6
# python manage.py seed_competitor_defaults --property-id 1000
# python manage.py seed_competitor_defaults --all


class Command(BaseCommand):
    help = 'Seed predefined competitor groups, categories, and mappings for one or more properties.'
    ...
    def handle(self, *args, **options):
        ...
        for property_obj in properties.order_by('id'):
            seed_default_competitors_for_property(property_obj, actor=None)
            self.stdout.write(
                self.style.SUCCESS(
                    f'Seeded competitor defaults for property {property_obj.code} - {property_obj.name}'
                )
            )