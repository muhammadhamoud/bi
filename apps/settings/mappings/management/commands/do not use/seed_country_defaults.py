...
from apps.settings.mappings.data_dontuse.country_defaults import seed_default_countries_for_property

# python manage.py seed_country_defaults --property-code P6
# python manage.py seed_country_defaults --property-id 1000
# python manage.py seed_country_defaults --all


class Command(BaseCommand):
    help = 'Seed predefined country groups, categories, mappings, and details for one or more properties.'
    ...
    def handle(self, *args, **options):
        ...
        for property_obj in properties.order_by('id'):
            seed_default_countries_for_property(property_obj, actor=None)
            self.stdout.write(
                self.style.SUCCESS(
                    f'Seeded country defaults for property {property_obj.code} - {property_obj.name}'
                )
            )